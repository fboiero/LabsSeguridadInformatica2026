...
def generar_manifiesto(directorio: Path, salida: Path) -> dict[str, str]:
    ...
    salida_res = salida.resolve()

    for ruta_archivo in directorio.rglob("*"):
        if not ruta_archivo.is_file():
            continue
        if ruta_archivo.resolve() == salida_res:
            continue

        ruta_posix = ruta_archivo.relative_to(directorio).as_posix()
        manifiesto[ruta_posix] = sha256_archivo(ruta_archivo)
...
def verificar_manifiesto(
    ...
    """
    ruta_manif_res = ruta_manifiesto.resolve()
    archivos_disco: dict[str, Path] = {}

    for ruta_archivo in directorio.rglob("*"):
        if not ruta_archivo.is_file():
            continue
        if ruta_archivo.resolve() == ruta_manif_res:
            continue
        ruta_posix = ruta_archivo.relative_to(directorio).as_posix()
        archivos_disco[ruta_posix] = ruta_archivo

    rutas_disco = set(archivos_disco.keys())
    rutas_manifiesto = set(manifiesto.keys())

    en_ambos = rutas_disco & rutas_manifiesto
    solo_disco = rutas_disco - rutas_manifiesto
    solo_manif = rutas_manifiesto - rutas_disco

    lista_ok: list[str] = []
    lista_modificado: list[str] = []

    for ruta_posix in sorted(en_ambos):
        digest_actual = sha256_archivo(archivos_disco[ruta_posix])
        if digest_actual == manifiesto[ruta_posix]:
            lista_ok.append(ruta_posix)
        else:
            lista_modificado.append(ruta_posix)

    return {
        ESTADO_OK: sorted(lista_ok),
        ESTADO_MODIFICADO: sorted(lista_modificado),
        ESTADO_FALTANTE: sorted(solo_manif),
        ESTADO_NUEVO: sorted(solo_disco),
    }
...
def calcular_mac(clave: bytes, mensaje: bytes, tag_esperado: str | None = None) -> tuple[str, bool | None]:
    ...
    """
    mac_obj = hmac.new(clave, mensaje, hashlib.sha256)
    tag_calculado = mac_obj.hexdigest()

    if tag_esperado is None:
        resultado: bool | None = None
    else:
        resultado = hmac.compare_digest(tag_calculado.lower(), tag_esperado.lower())

    return tag_calculado, resultado
